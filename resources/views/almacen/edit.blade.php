        ...
        <form action="{{ route('almacenes.update', $almacen->codigo_almacen) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <input type="text" name="descripcion" id="descripcion" value="{{ old('descripcion', $almacen->descripcion) }}" class="w-full px-4 py-2 border rounded-lg focus:ring-primary focus:border-primary {{ $errors->has('descripcion') ? 'border-red-500' : 'border-gray-300' }}" placeholder="Nombre o descripción del almacén" maxlength="100" required>
                    @error('descripcion')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <input type="text" name="direccion" id="direccion" value="{{ old('direccion', $almacen->direccion) }}" class="w-full px-4 py-2 border rounded-lg focus:ring-primary focus:border-primary {{ $errors->has('direccion') ? 'border-red-500' : 'border-gray-300' }}" placeholder="Ubicación física del almacén" maxlength="200">
                    @error('direccion')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="col-span-1 md:col-span-2">
                    ...
                    <input type="text" name="responsable" id="responsable" value="{{ old('responsable', $almacen->responsable) }}" class="w-full px-4 py-2 border rounded-lg focus:ring-primary focus:border-primary {{ $errors->has('responsable') ? 'border-red-500' : 'border-gray-300' }}" placeholder="Persona encargada del almacén" maxlength="100">
                    @error('responsable')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="flex justify-end space-x-3 pt-6">
                ...
                <button type="submit" class="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-primary-dark shadow-md transition font-semibold">Actualizar Almacén</button>
            </div>
        </form>
    </div>
</div>
