        ...
        <form method="GET" action="{{ route('requerimientos_materiales.index') }}" class="w-full flex flex-col md:flex-row gap-4">
            ...
            <div class="flex items-center gap-2">
                <div class="flex flex-col">
                    ...
                    <input type="date" name="fecha_desde" value="{{ $fecha_desde }}" class="border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 outline-none" max="{{ date('Y-m-d') }}">
                </div>
                <div class="flex flex-col">
                    <label class="text-[10px] uppercase text-slate-500 font-bold mb-1">Fecha Hasta</label>
                    <input type="date" name="fecha_hasta" value="{{ $fecha_hasta }}" class="border border-slate-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 outline-none" max="{{ date('Y-m-d') }}">
                </div>
            </div>
            ...
            <div class="flex-1 flex flex-col">
                ...
                <div class="relative w-full">
                    ...
                    <input type="text" name="codigo" value="{{ request('codigo') }}" placeholder="Buscar por código..." 
                        class="w-full pl-10 pr-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 text-sm outline-none">
                </div>
            </div>
            ...
            <div class="flex items-end gap-2">
                ...
                <a href="{{ route('requerimientos_materiales.index') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-lg text-sm font-bold transition-colors border border-slate-300">
                    Limpiar
                </a>
            </div>
        </form>
            ...
            <table class="w-full text-left whitespace-nowrap">
                <thead>
                    <tr class="bg-slate-900 text-slate-200 text-xs uppercase tracking-wider font-bold">
                        <th class="px-4 md:px-6 py-3.5 border-r border-slate-800">Código / Fecha</th>
                        <th class="px-4 md:px-6 py-3.5 border-r border-slate-800">Productos</th>
                        <th class="px-4 md:px-6 py-3.5 border-r border-slate-800">Creado Por</th>
                        <th class="px-4 md:px-6 py-3.5 border-r border-slate-800 text-center">Estado</th>
                ...
                <tbody class="divide-y divide-slate-100 text-xs md:text-sm">
                    @forelse ($requerimientos as $req)
                        <tr class="hover:bg-slate-50/50 transition">
                            <td class="px-4 md:px-6 py-3 md:py-4">
                                ...
                                <div class="text-xs text-slate-500">{{ \Carbon\Carbon::parse($req->fecha_requerimiento)->format('d/m/Y') }}</div>
                            </td>
                            <td class="px-4 md:px-6 py-3 md:py-4">
                                <div class="text-slate-800 font-medium">
                                    {{ $req->detalles->pluck('codigo_producto')->unique()->take(3)->implode(', ') }}
                                    @if($req->detalles->pluck('codigo_producto')->unique()->count() > 3)
                                        <span class="text-xs text-slate-400">(+{{ $req->detalles->pluck('codigo_producto')->unique()->count() - 3 }})</span>
                                    @endif
                                </div>
                                <div class="text-xs text-slate-500">{{ $req->detalles->count() }} línea(s)</div>
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-slate-500 italic">No se encontraron requerimientos.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
