    ...
    <div class="bg-white p-3 md:p-4 rounded-xl shadow-md mb-6">
        <form action="{{ route('mermas.index') }}" method="GET" class="flex flex-col md:flex-row gap-3">
            <div class="relative w-full md:w-48">
                <input type="date" name="fecha" value="{{ request('fecha') }}" class="w-full px-4 py-2 md:py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-sm md:text-base outline-none text-slate-600" max="{{ date('Y-m-d') }}">
            </div>
            <div class="relative flex-1">
                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                    <i class="fas fa-search text-slate-400"></i>
                </div>
                <input type="text" name="search" value="{{ request('search') }}" class="w-full pl-10 pr-4 py-2 md:py-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-sm md:text-base outline-none" placeholder="Buscar por código o producto...">
            </div>
            ...
            <a href="{{ route('mermas.index') }}" class="bg-slate-100 hover:bg-slate-200 text-slate-600 px-4 md:px-6 py-2 md:py-2.5 rounded-lg border border-slate-300 transition font-medium text-sm md:text-base flex items-center justify-center">
                ...
                <tbody class="divide-y divide-slate-100 text-xs md:text-sm">
                    @forelse ($mermas as $m)
                        <tr class="hover:bg-slate-50/50 transition">
                            ...
                            <td class="px-4 md:px-6 py-3 md:py-4 text-slate-800 font-medium whitespace-nowrap">
                            ...
                            <td class="px-4 md:px-6 py-3 md:py-4 text-slate-800">
                                {{ $m->codigo_almacen }}
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8" class="px-6 py-10 text-center text-slate-500 italic">No se encontraron registros de mermas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
<!-- Modal Visualizar -->
<div id="modalVisualizar" class="fixed inset-0 z-50 hidden overflow-y-auto bg-slate-900 bg-opacity-50 flex items-center justify-center">
    <div class="bg-white rounded-xl shadow-xl w-full max-w-4xl m-4 max-h-[90vh] flex flex-col">
        <div class="px-6 py-4 border-b flex justify-between items-center shrink-0">
            <h3 class="text-lg font-bold text-slate-800">Detalle de Merma</h3>
            <button onclick="cerrarModal()" class="text-slate-400 hover:text-slate-600"><i class="fas fa-times text-xl"></i></button>
        </div>
        ...
        <div class="px-6 py-4 border-t bg-slate-50 flex justify-end shrink-0 rounded-b-xl">
            <button onclick="cerrarModal()" class="bg-slate-200 hover:bg-slate-300 text-slate-800 font-semibold py-2 px-4 rounded-lg shadow-sm transition">Cerrar</button>
        </div>
    </div>
</div>
...
<script>
    function verMerma(id) {
        ...
        document.getElementById('modalContenido').innerHTML = '<div class="text-center py-10"><i class="fas fa-spinner fa-spin text-3xl text-primary mb-3"></i><p class="text-slate-500">Cargando detalles...</p></div>';
