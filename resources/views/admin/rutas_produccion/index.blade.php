...
<div class="container mx-auto max-w-5xl pb-8 md:pb-10">
    <div class="flex justify-between items-center mb-6 gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-800">Rutas de Producción</h1>
            <p class="text-xs sm:text-sm text-gray-600">Asigne los procesos correspondientes a cada producto en proceso (PEP).</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="col-span-1 md:col-span-1 bg-white rounded-xl shadow-md p-6 border-t-4 border-primary">
            <h3 class="text-lg font-bold text-gray-700 mb-4">Seleccionar Producto</h3>
                ...
                @foreach($productos as $producto)
                    <li>
                        <button type="button" class="w-full text-left px-4 py-3 rounded-lg border flex flex-col transition-colors
                            ...
                            <span class="font-bold text-sm text-gray-800">{{ $producto->descripcion }}</span>
        ...
        <div class="col-span-1 md:col-span-2 bg-white rounded-xl shadow-md p-6 border-t-4 border-primary hidden" id="procesos-panel">
            <h3 class="text-lg font-bold text-gray-700 mb-2">Procesos Asignados</h3>
            <p class="text-sm text-gray-600 mb-6" id="producto-selected-name"></p>
            ...
            <form action="{{ route('admin.rutas_produccion.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
                    @foreach($procesos as $proceso)
                        <label class="flex items-start p-3 border rounded-lg hover:bg-gray-50 cursor-pointer transition-colors">
                            <input type="checkbox" name="procesos[]" value="{{ $proceso->codigo }}" class="proceso-checkbox mt-1 h-4 w-4 text-primary focus:ring-primary border-gray-300 rounded">
                            <div class="ml-3">
                                <span class="block text-sm font-medium text-gray-800">{{ $proceso->descripcion }}</span>
                            </div>
                        </label>
                    @endforeach
                </div>
        ...
        <div class="col-span-1 md:col-span-2 bg-white rounded-xl shadow-md p-6 flex flex-col items-center justify-center text-center text-gray-500" id="empty-panel">
            <i class="fas fa-route text-5xl mb-4 text-gray-300"></i>
            <h3 class="text-lg font-bold text-gray-600">Seleccione un producto</h3>
