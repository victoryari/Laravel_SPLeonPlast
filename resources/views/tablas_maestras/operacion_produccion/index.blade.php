...
<div class="container mx-auto pb-8 md:pb-10">
    <x-page-header title="Operaciones de Producción" subtitle="Gestión de etapas y operaciones del proceso">
        <x-slot:actions>
            <a href="{{ route('operaciones_produccion.create') }}" class="btn-primary">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse ($operaciones as $op)
                            <tr class="hover:bg-slate-50/80 transition duration-150">
                                <td class="px-4 md:px-6 py-2.5 font-bold text-xs md:text-sm text-slate-900 whitespace-nowrap">{{ $op->codigo }}</td>
                                <td class="px-4 md:px-6 py-2.5 text-xs md:text-sm font-medium text-slate-800 uppercase">{{ $op->descripcion }}</td>
                                <td class="px-4 md:px-6 py-2.5 text-center whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-1.5 md:gap-2">
                                        <a href="{{ route('operaciones_produccion.edit', $op->codigo) }}" class="inline-flex items-center justify-center w-8 h-8 text-primary bg-primary-50 hover:bg-primary hover:text-white rounded-lg transition-all shadow-2xs" title="Editar">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="py-10">
                                    <x-empty-state icon="fa-tools" message="No se encontraron registros." />
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
