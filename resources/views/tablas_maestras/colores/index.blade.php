...
<div class="container mx-auto pb-8 md:pb-10">
    <x-page-header title="Maestro de Colores" subtitle="Catálogo de colores para productos y materiales">
        <x-slot:actions>
            <a href="{{ route('colores.create') }}" class="btn-primary">
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse ($colores as $color)
                            <tr class="hover:bg-slate-50/80 transition duration-150">
                                <td class="px-4 md:px-6 py-2.5 font-bold text-xs md:text-sm text-slate-900 whitespace-nowrap">{{ $color->codigo }}</td>
                                <td class="px-4 md:px-6 py-2.5 text-xs md:text-sm font-medium text-slate-800 uppercase">{{ $color->descripcion }}</td>
                                <td class="px-4 md:px-6 py-2.5 text-center whitespace-nowrap">
                                    <div class="flex items-center justify-center gap-1.5 md:gap-2">
                                        <a href="{{ route('colores.edit', $color->codigo) }}" class="inline-flex items-center justify-center w-8 h-8 text-primary bg-primary-50 hover:bg-primary hover:text-white rounded-lg transition-all shadow-2xs" title="Editar">
                        ...
                        @empty
                            <tr>
                                <td colspan="3" class="py-10">
                                    <x-empty-state icon="fa-palette" message="No se encontraron colores registrados." />
                                </td>
                            </tr>
                        @endforelse
                    </tbody>
