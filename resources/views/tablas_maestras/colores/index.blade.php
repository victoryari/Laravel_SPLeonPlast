...
<div class="container mx-auto pb-8 md:pb-10">
    <div class="flex justify-between items-center mb-6 gap-4">
        <div>
            <h1 class="text-xl sm:text-2xl font-bold text-gray-800">Maestro de Colores</h1>
            <p class="text-xs sm:text-sm text-gray-600">Catálogo de colores para productos y materiales</p>
        </div>
        <a href="{{ route('colores.create') }}" class="shrink-0 flex items-center justify-center bg-primary hover:bg-primary-dark text-white font-semibold py-2 px-3 sm:px-4 rounded-lg shadow transition">
            <i class="fas fa-plus"></i>
            <span class="hidden sm:inline ml-2">Nuevo Color</span>
        </a>
    </div>
                    ...
                    <tbody class="divide-y divide-gray-100 text-xs md:text-sm">
                        @forelse ($colores as $color)
                            <tr class="hover:bg-slate-50/50 transition">
                                <td class="px-4 md:px-6 py-3 md:py-4 font-bold text-gray-900">{{ $color->codigo }}</td>
                                <td class="px-4 md:px-6 py-3 md:py-4 text-gray-700">{{ $color->descripcion }}</td>
                                <td class="px-4 md:px-6 py-3 md:py-4 text-center">
                                    <div class="flex items-center justify-center gap-2 md:gap-3">
                                        <a href="{{ route('colores.edit', $color->codigo) }}" class="inline-flex items-center justify-center w-8 h-8 md:w-10 md:h-10 text-primary bg-primary-50 hover:bg-primary hover:text-white rounded-lg transition" title="Editar">
                                            <i class="fas fa-edit text-sm md:text-lg"></i>
                                        </a>
                                        <form action="{{ route('colores.destroy', $color->codigo) }}" method="POST" class="inline-block" onsubmit="return confirm('¿Está seguro de desactivar este color?');">
                                            @csrf @method('DELETE')
