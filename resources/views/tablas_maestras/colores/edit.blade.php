...
<div class="container mx-auto max-w-2xl">
    <x-page-header title="Modificar Color" subtitle="Actualizando registro: {{ $color->codigo }}" />

    <x-card class="p-6 md:p-8">
        <form action="{{ route('colores.update', $color->codigo) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <x-form-group label="Descripción" required :error="$errors->first('descripcion')">
                <input type="text" name="descripcion" id="descripcion" value="{{ old('descripcion', $color->descripcion) }}" class="input-field @error('descripcion') border-red-500 @enderror" required>
