...
<div class="container mx-auto pb-10 max-w-2xl">
    <div class="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
            <h1 class="text-xl md:text-2xl font-bold text-gray-800">Modificar Molde</h1>
            <p class="text-sm text-gray-600">Actualizando molde: <span class="font-bold text-blue-600">{{ $molde->codigo }}</span></p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden">
        <form action="{{ route('moldes.update', $molde->codigo) }}" method="POST" class="p-6 md:p-8">
            @csrf
            @method('PUT')
            <div class="space-y-6">
                <div>
                    <label class="block text-sm font-bold text-gray-400 mb-2">Código del Molde</label>
                    <input type="text" value="{{ $molde->codigo }}" 
