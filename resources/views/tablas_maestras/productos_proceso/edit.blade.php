            ...
            <form action="{{ route('productos_proceso.update', $producto->codigo) }}" method="POST" class="space-y-6">
                ...
                <div class="pt-4 border-t border-slate-100">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <span class="text-xs text-slate-500">(seleccione los moldes que se usan para fabricar este producto)</span>
                    </div>
                    ...
                    @if($moldes->count() > 0)
                            ...
                            @foreach($moldes as $molde)
                                <label class="flex items-center gap-3 p-2 rounded-md hover:bg-white cursor-pointer transition molde-item" data-desc="{{ strtolower($molde->codigo . ' ' . $molde->descripcion) }}">
                                    <input type="checkbox" name="moldes[]" value="{{ $molde->codigo }}"
                                        {{ in_array($molde->codigo, old('moldes', $moldes_vinculados)) ? 'checked' : '' }}
                        ...
                        <p class="text-xs text-slate-500 mt-2">
                            <span id="conteo_moldes">{{ count($moldes_vinculados) }}</span> molde(s) seleccionado(s) de {{ $moldes->count() }} disponibles
                        </p>
                    @else
