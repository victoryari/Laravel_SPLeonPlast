...
<div class="container mx-auto pb-10 max-w-5xl">
    <x-page-header title="Editar Rol" subtitle="Modificando accesos para: {{ $role->nombre }}">
        <x-slot:actions>
                ...
                <div class="card p-6 sticky top-6">
                    ...
                    <div class="space-y-4">
                        <x-form-group label="Nombre del Rol" required :error="$errors->first('nombre')">
                            <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $role->nombre) }}" class="input-field @error('nombre') border-red-500 @enderror" {{ $role->nombre === 'Administrador' ? 'readonly' : 'required' }}>
                        ...
                        <x-form-group label="Descripción">
                            <textarea name="descripcion" id="descripcion" rows="3" class="input-field">{{ old('descripcion', $role->descripcion) }}</textarea>
